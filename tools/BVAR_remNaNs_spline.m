function [X,indNaN] = BVAR_remNaNs_spline(X,options)
% This function treats NaNs in dataset for use in DFM
%
% Code from Banbura, M., Giannone D., and Reichlin, L. (2010). "Nowcasting", 
% in Clements, M. P., and Hendry, D. F. (editors), Oxford Handbook on 
...
switch options.method
    case 1  % replace all the missing values
        for i = 1:N  % loop through columns
            x = X(:,i); 
            x(indNaN(:,i))= nanmedian(x);  % Replace missing values series median 
