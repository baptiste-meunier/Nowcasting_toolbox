function [w, A, C, sbc, fpe, th] = BEQ_arfit(v, pmin, pmax, selector, no_const)
% This script does a stepwise least squares estimation of multivariate AR model
%
...
%           neum@cma.univie.ac.at
  ...
  [R, scale]   = BEQ_arqr(v, pmax, mcor);

  % compute approximate order selection criteria for models 
  % of order pmin:pmax
  [sbc, fpe]   = BEQ_arord(R, m, mcor, ne, pmin, pmax);
